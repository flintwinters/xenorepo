...
When a command sits inside colored pane chrome, derive its opaque face, border, highlight, and shadow colors from that chrome's base tone. Keep enough dark contrast for the command to remain distinct, but do not reveal the chrome through transparent button layers.
...
Inputs, text areas, and selectors use compact recessed dark fields, fine neutral borders, modest padding, and visible labels. Text areas have layout-owned dimensions and are never user-resizable. Their depth must be quieter than the command buttons and must not introduce a second, harsher bevel language. Disabled controls remain legible but subdued. Use a clear high-contrast treatment on every interactive control.
...
Render tables with the established ledger treatment and semantic column roles. Compact identity and numeric columns to their content and allocate remaining width to prose. Put exactly one logical value in each cell: never deliberately stack labels, paths, metadata, or explanations vertically within a cell. Avoid wrapping through meaningful widths, but allow prose to wrap when the viewport requires it. Use aligned columns, thin cell rules, clipped overflow, and sticky headings where useful. Render charts directly on ruled dark plotting fields with simple luminous marks and restrained shading. Render meters as narrow recessed tracks with polished bright fills. Keep semantic colors consistent for selected, nominal, caution, critical, and secondary states. Promote the treatment into shared typed UI only after independent Preact consumers prove the same boundary.
