    ...
    css`
      ...
      * {
      ...
      .workspace {
        display: grid;
        min-height: 0;
        grid-template-columns: minmax(500px, 2.2fr) minmax(260px, 1fr);
        gap: 1px;
        background: #111;
      }
      x-console-pane {
      ...
      .day.selected {
        background: #343324;
        box-shadow: inset 0 0 0 2px #d79921;
      }
      .day.today .number {
        color: #1d2021;
        background: #fabd2f;
      ...
      .day.drop {
        background: #354a44;
        box-shadow: inset 0 0 0 2px #83a598;
      }
      .number {
        position: absolute;
        top: 3px;
      ...
      @media (max-width: 720px) {
        .workspace {
          ...
          grid-template-rows: minmax(330px, 52vh) minmax(240px, 1fr);
          overflow: auto;
        }
        .grid {
    ...
    >
      <section class="dialog" role="dialog" aria-modal="true" aria-labelledby="editor-title">
        ...
        <form @submit=${this.save}>
          ...
          ><label>Date<input name="date" type="date" required .value=${item?.date ?? this.selected} /></label
          ...
          <div class="times">
            <label
              >Start<input
                ...
                .value=${shortTime(item?.start_time ?? "09:00")} /></label
            ><label
              >End<input
                name="end_time"
                type="time"
                ?required=${!this.allDay}
                ?disabled=${this.allDay}
                .value=${shortTime(item?.end_time ?? "10:00")}
            /></label>
          </div>
          <label>Location<input name="location" maxlength="2000" .value=${item?.location ?? ""} /></label
          ><label>Notes<textarea name="notes" maxlength="2000" .value=${item?.notes ?? ""}></textarea></label>
          <div class="actions">
            ${item
              ? html`<x-command-button class="delete-action" @click=${() => this.removeEvent(item)}
                  >DELETE</x-command-button
                >`
              : nothing}<x-command-button @click=${this.closeEditor}>CANCEL</x-command-button
