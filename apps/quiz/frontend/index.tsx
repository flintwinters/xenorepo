...
interface Item { dimension: Dimension; text: string; }
interface Response { index: number; value: number; }
interface InventoryState { index: number; responses: Response[]; }
...
class Inventory extends Component<Record<string, never>, InventoryState> {
  ...
  private restart = (): void => { this.setState({ index: 0, responses: [] }); };
  ...
  override render() {
    ...
    const scores = Object.fromEntries(dimensions.map((dimension) => [dimension, score(responses, dimension)])) as
    ...
    return <main class="inventory" aria-label="Working Style Inventory">
      <header class="utility">
        <span class="brand">◈ WORKING STYLE</span><span class="context">SELF-REFLECTION / EIGHT ITEMS</span>
        <span class="live">{complete ? "COMPLETE" : "READY"}</span><button onClick={this.restart}>RESTART</button>
      </header>
      <section class="grid">
        <aside class="pane bank"><h2 class="pane-title">ITEMS</h2><ol class="item-list">
          {items.map((_, itemIndex) => <li class={itemIndex === index && !complete ? "active" : ""}>
            <span class="number">{String(itemIndex + 1).padStart(2, "0")}</span>
            <span>{responses[itemIndex] ? "RECORDED" : "PENDING"}</span>
          </li>)}
        </ol></aside>
        <section class="pane arena"><h2 class="pane-title">RESPONSE</h2><div class="question-area">
          <div class="progress"><span>{complete ? "PROFILE COMPLETE" : `ITEM ${index + 1} / ${items.length}`}</span>
            <span>{complete ? strongest : current.dimension}</span></div>
          <h1 class="prompt">{complete
            ? `Your strongest current signal is ${strongest.toLowerCase()}.`
            : current.text}</h1>
          <div class="answers" aria-live="polite">{complete
            ? <button onClick={this.restart}>START AGAIN</button>
            : labels.map((label, answerIndex) => <button
                class={`answer ${responses[index]?.value === answerIndex + 1 ? "selected" : ""}`}
                onClick={() => this.record(answerIndex + 1)}>
              <span class="keynum">{answerIndex + 1}</span>{label}
            </button>)}</div>
        </div></section>
        <aside class="pane profile"><h2 class="pane-title">PROFILE</h2><div class="profile-card"><dl>
          {dimensions.map((dimension) => <><dt>{dimension}</dt>
            <dd>{scores[dimension] === null ? "—" : `${scores[dimension]}%`}</dd></>)}
        </dl><div class="meter"><i style={{ width: `${responses.length / items.length * 100}%` }} /></div>
          <div class="status-message">{complete
            ...
            : responses[index]
              ...
              : <><strong>There are no right answers.</strong>Choose the response that fits you best.</>}</div>
        </div></aside>
        <section class="pane review"><h2 class="pane-title">RESPONSE LOG</h2>
          <div class="pane-body"><ol class="review-list">
          {responses.length ? responses.map((response, responseIndex) => <li>
          ...
          </li>) : <li><span class="muted">—</span><span class="muted">Your responses will appear here.</span>
            <span class="result">WAITING</span></li>}
        </ol></div></section>
      </section>
      <footer class="status"><span><strong>● PRIVATE LOCAL SESSION</strong>
        {" · NOT A CLINICAL ASSESSMENT"}</span>
        <span class="hint">1–5 SELECT · ENTER CONTINUE · R RESTART</span>
        <span>{responses.length} RECORDED</span></footer>
    </main>;
  }
}
