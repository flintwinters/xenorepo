...
class Microblog extends Component<Record<string, never>, State> {
  override state: State = {
    ...
    posts: [],
    body: "",
    authMessage: "ENTER ACCOUNT CREDENTIALS",
    authFailed: false,
  ...
  private submitAuth = (event: SubmitEvent): void => {
    ...
    this.setState({
      authMessage: signingIn ? "SIGNING IN…" : "REGISTERING…",
      authFailed: false,
      offerRegistration: false,
    });
    void authenticate(this.state.mode, { handle, password: password.value })
      .then((session) => {
        password.value = "";
  ...
  private accountPane(): preact.JSX.Element {
    ...
    return (
      <aside class={`pane account${this.state.mobileAccount ? " mobile-open" : ""}`} aria-labelledby="account-title">
        <div class="title blue" id="account-title">
          <span class="plaque">01</span>ACCOUNT LINK
        </div>
        <div class="account-body">
          {session.authenticated && session.account ? (
            <div id="signed" class="signed">
              <div>LINK ACTIVE</div>
              <p class="handle" id="signedHandle">
                @{session.account.handle}
              </p>
              <CommandButton id="logout" onClick={this.signOut}>
                SIGN OUT
              </CommandButton>
            </div>
          ) : (
            <div id="guest">
              ...
              <form id="authForm" onSubmit={this.submitAuth}>
                ...
                <CommandButton id="authSubmit">{mode === "login" ? "SIGN IN" : "REGISTER"}</CommandButton>
              </form>
              <div
                class={`auth-message${this.state.authFailed ? " error" : ""}`}
                id="authMessage"
                role="alert"
                aria-live="assertive"
              >
                {this.state.authMessage}
                {this.state.offerRegistration && (
                  <>
                    <br />
                    <CommandButton type="button" onClick={() => this.setMode("register")}>
    ...
    return (
      <>
        {this.state.posts.map((post) => (
          <article class="post" data-id={post.id} key={post.id}>
            <div class="post-head">
              <span class="post-author">@{post.author}</span>
              <span>#{post.id}</span>
              <time class="post-time">{new Date(post.created_at).toLocaleString()}</time>
            </div>
            <div class="post-body">{post.body}</div>
            <div class="post-actions">
              <CommandButton
                class={`like${post.liked_by_me ? " liked" : ""}`}
                disabled={!this.state.session.authenticated}
                onClick={() => this.like(post)}
              >
                {post.liked_by_me ? "UNLIKE" : "LIKE"}
              </CommandButton>
              <span>
                {post.like_count} SIGNAL{post.like_count === 1 ? "" : "S"}
              </span>
            </div>
          </article>
        ))}
  ...
  override render(): preact.JSX.Element {
    ...
    return (
      <div class="frame">
        <header class="rail">
          <span class="brand">WIRE/98</span>
          <span class="summary">PUBLIC MICROBLOG CIRCUIT</span>
          <span id="identity">{active ? `@${this.state.session.account!.handle}` : "GUEST"}</span>
          <CommandButton
            class="mobile-account"
            id="accountCommand"
            onClick={() => this.setState((state) => ({ mobileAccount: !state.mobileAccount }))}
          >
            ACCOUNT
          </CommandButton>
          <CommandButton id="refresh" onClick={() => void this.refresh()}>
            REFRESH
          </CommandButton>
        </header>
        <main class="workspace">
          ...
          <section class="pane main" aria-labelledby="feed-title">
            <div>
              <div class="title" id="feed-title">
                <span class="plaque">02</span>TRANSMISSION STREAM
              </div>
              <form class="composer" id="postForm" onSubmit={this.submitPost}>
                ...
                <textarea
                  id="body"
                  maxlength={280}
                  placeholder={active ? "What is on the wire?" : "Sign in to publish."}
                  disabled={!active}
                ...
                <div class="compose-row">
                  <CommandButton id="publish" disabled={!active}>
                    PUBLISH
                  </CommandButton>
                  <span id="composeHint">{active ? "READY" : "AUTHENTICATION REQUIRED"}</span>
                  <output class="counter" id="counter">
                    {this.state.body.length}/280
                  </output>
                </div>
              </form>
            </div>
        ...
        <footer class="rail status">
          <span class="signal">● ONLINE</span>
          <span id="message" class={this.state.failed ? "error" : ""}>
            {this.state.message}
          </span>
          <span class="summary" />
          <span class="low">LAST SYNC:</span>
          <time id="lastSync">{this.state.lastSync}</time>
        </footer>
      </div>
    );
  }
