  ...
  override render() {
    ...
    const header = (
      <UtilityRail>
        ...
        <span class="context">ARITHMETIC / DESK</span>
        <span class="mode">READY</span>
      </UtilityRail>
    );
    const footer = (
      <StatusRail class="status">
        <span>4-FUNCTION</span>
        <span class="hint">KEYBOARD ENABLED · ESC CLEAR</span>
      </StatusRail>
    );
    return (
      <ConsoleShell class="calculator-shell" header={header} footer={footer}>
        <section class="workspace">
          <ConsolePane class="calculator-pane display-pane" title="CALCULATION" tone="green">
            <div class="calculator">
              <div class="readout" aria-live="polite">
                ...
                <output class={calculator.error ? "error" : ""} aria-label="Display">
                  {calculator.display}
                </output>
              </div>
            </div>
          </ConsolePane>
          <ConsolePane class="calculator-pane keypad-pane" title="KEYPAD">
            <div class="calculator">
              <div class="keys">
                {this.key("C", "function")}
                {this.key("±", "function")}
          ...
          <section class="side">
            <ConsolePane title="MODES" tone="orange">
              <div class="tabs" role="tablist" aria-label="Calculation mode">
                {(["standard", "scientific"] as const).map((item) => (
                  <CommandButton
                    role="tab"
                    aria-selected={mode === item}
                    data-mode={item}
                    onClick={() => this.commit({ ...this.state, mode: item })}
                  >
                    {item.toUpperCase()}
                  </CommandButton>
                ))}
                ...
                <div class="scientific-keys">
                  {["sin", "cos", "tan", "ln", "log", "square", "pi"].map((action) => (
                    <CommandButton
                      ...
                      aria-label={action}
                      onClick={() => this.scientific(action)}
                    >
                      {action}
                    </CommandButton>
                  ))}
            ...
            <ConsolePane title="HISTORY" tone="purple">
              <ol class="history">
                {ledger.map((entry) => (
                  <li>{entry}</li>
                ))}
