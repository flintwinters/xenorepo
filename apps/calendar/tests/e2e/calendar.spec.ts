...
test("[acceptance] creates, edits, drags, reloads, and deletes a durable event", async ({ page }, testInfo) => {
  ...
  expect(next!.x + next!.width).toBeLessThanOrEqual(chrome!.x + chrome!.width);
  expect(Math.abs(next!.y + next!.height / 2 - (chrome!.y + chrome!.height / 2))).toBeLessThan(2);
  const title = `Commitment ${Date.now()}`;
  await page.getByRole("button", { name: "ADD", exact: true }).click();
  await page.getByLabel("Title").fill(title);
  await page.getByLabel("Location").fill("Studio B");
  await page.getByLabel("Notes").fill("Bring the planning ledger");
  ...
  await page.getByLabel("Start").fill("13:15");
  await page.getByLabel("End").fill("14:45");
  await page.getByRole("button", { name: "SAVE", exact: true }).click();
  const edited = `${title} edited`;
  await expect(page.locator(".agenda-row").filter({ hasText: edited })).toContainText("13:15");
  const cells = page.locator(".day");
  const sourceIndex = await cells.evaluateAll(
  ...
  if (modality === "touch") {
    ...
    const source = await page.locator(".agenda-row").filter({ hasText: edited }).locator(".drag-handle").boundingBox(),
    ...
    await touchPath(page, [
      ...
      { x: source.x + source.width / 2 + 12, y: source.y + source.height / 2 },
  ...
  await testInfo.attach("input-evidence.json", {
    body: JSON.stringify({ schemaVersion: 1, modality, records: evidence }, null, 2),
    contentType: "application/json",
  });
  const authoritative = await page.request.get(
    `/api/calendar?start=${destinationDate}&end=${nextDate(destinationDate!)}`,
  );
  expect(authoritative.ok()).toBe(true);
  expect((await authoritative.json()).events).toEqual([
    expect.objectContaining({ title: edited, date: destinationDate, location: "Studio B" }),
  ]);
  await page.reload();
  await page.locator(`.day[data-date="${destinationDate}"]`).click({ position: { x: 5, y: 5 } });
  await expect(page.locator(".agenda-row").filter({ hasText: edited })).toBeVisible();
  await page.locator(".agenda-row").filter({ hasText: edited }).click();
