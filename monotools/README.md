...
The command records the repository deletion immediately in one path-scoped Git
commit, leaving unrelated changes outside that commit. It deliberately does not
delete an independently hosted remote repository. Reverting the commit restores
...
Xenorepo's gitlink. The routine bootstraps missing locked dependencies before these
gates. Fresh checkouts initialize all declared app submodules through `uv run manage.py bootstrap`.
