...
export async function loadCockpit(): Promise<
  [Overview, ModuleFact[], TreeNode, RepositoryHistory, Snapshot[]]
> {
  const [overview, modules, tree, repositoryHistory, snapshots] = await Promise.all([
    api.GET("/api/overview"), api.GET("/api/modules"), api.GET("/api/tree"),
    api.GET("/api/repository-history"), api.GET("/api/history"),
  ]);
