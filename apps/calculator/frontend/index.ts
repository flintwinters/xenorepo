...
interface SavedState { state: CalculatorState; mode: CalculationMode; ledger: string[]; }
...
class CalculatorApp extends LitElement {
  ...
  private scientific(action: string): void {
    ...
    const operations: Record<string, () => number> = {
      ...
      ln: () => Math.log(value), log: () => Math.log10(value), square: () => value ** 2,
      pi: () => Math.PI,
    };
    const result = operations[action]();
    const display = Number.isFinite(result) ? String(Number.parseFloat(result.toPrecision(12))) : "Error";
    this.state = { ...initialState(), display, replaceDisplay:true, error:display === "Error",
  ...
  private key(label: string, className = ""): unknown {
    ...
    return html`<x-command-button class="calc-key ${className}" label=${spoken[label] ?? label}
  ...
  static styles = css`
    ...
    * { box-sizing:border-box; }
    ...
    .workspace { display:grid; grid-template-columns:minmax(300px,420px) minmax(180px,260px); grid-template-rows:auto minmax(0,1fr); place-content:center; min-height:0; padding:clamp(14px,4vw,48px); gap:1px; background:radial-gradient(circle at 50% 35%,#32302f,#1d2021 66%); }
    x-console-pane { min-height:0; border:1px solid #111; box-shadow:0 14px 34px rgb(0 0 0/.45); }
    .display-pane { grid-column:1/3; } .keypad-pane { grid-column:1; grid-row:2; }
    .side { display:grid; grid-template-rows:auto minmax(0,1fr); min-height:0; }
    ...
    .tabs button { flex:1; padding:5px; color:#a89984; font:inherit; background:#35312f; border:1px solid #928374; }
    ...
    x-command-button { height:52px; font-size:18px; --console-button-border:#a89984; }
    x-command-button.function { color:#fabd2f; }
    x-command-button.operator { color:#8ec07c; --console-button-border:#83a598; }
    ...
    @media(max-width:520px) {
      ...
      x-console-pane { width:100%; border:0; box-shadow:none; }
        ...
        <section class="side">
          <x-console-pane title="MODES" tone="orange">
            <div class="tabs" role="tablist" aria-label="Calculation mode">
              ...
              <button role="tab" aria-selected=${this.mode === "scientific"}
                @click=${() => { this.mode="scientific"; this.persist(); }} data-mode="scientific">SCIENTIFIC</button>
            </div>
            ${this.mode === "scientific" ? html`<div class="scientific-keys">
              ${["sin","cos","tan","ln","log","square","pi"].map((action) => html`
                <x-command-button class="calc-key scientific" label=${action}
