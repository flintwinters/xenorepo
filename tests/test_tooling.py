...
"""
    ...
    def test_worminal_uses_xterm_with_one_local_shell_socket_per_window(self) -> None:
        ...
        self.assertIn("fontSize: 9, lineHeight: 1, letterSpacing: 0", source)
        self.assertNotIn("lineHeight: 0.85", source)
        ...
        self.assertIn('window.addEventListener("keyup", this.handleSuperKeyUp, { capture: true })', source)
        self.assertIn('event.key === "Meta"', source)
        self.assertIn("if (openShell) this.spawn()", source)
