...
test("[acceptance] creates, edits, drags, archives, restores, and reloads durable work",
  ...
  await expect(page.getByLabel("Priority")).toHaveCount(0);
  const tags = cardDetails.getByLabel("Tags");
  await expect(tags.locator("option", { hasText: "durable" })).toBeVisible();
  await tags.selectOption(["acceptance", "durable"]);
  ...
  const tagColorInput = tagColor.getByLabel("Tag color");
  await tagColorInput.fill("#85a");
  await tagColorInput.locator("xpath=ancestor::form").getByRole("button", { name: "SAVE COLOR" }).click();
  ...
  const acceptanceTag = page.locator(".tag-catalog tbody tr").filter({ hasText: /^acceptance/ });
  ...
  const createdTagRow = page.locator(".tag-catalog tbody tr").filter({ hasText: new RegExp(`^${createdTag}`) });
