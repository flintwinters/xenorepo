...
export class WorminalDesktop extends LitElement {
  static properties = { windows: { state: true }, clock: { state: true }, shortcuts: { state: true }, settingsOpen: { state: true }, currentAccessPassword: { state: true }, newAccessPassword: { state: true }, confirmedAccessPassword: { state: true }, settingsError: { state: true }, settingsSaving: { state: true } };
  ...
  constructor() { super(); this.windows = []; this.clock = "--:--:--"; this.shortcuts = [this.defaultShortcut()]; this.settingsOpen = false; this.currentAccessPassword = ""; this.newAccessPassword = ""; this.confirmedAccessPassword = ""; this.settingsError = ""; this.settingsSaving = false; }
  ...
  static styles = [unsafeCSS(xtermCss), css`
    ...
    * { box-sizing: border-box; } x-console-shell { height: 100%; }
    ...
    .titlebar button { width: 24px; padding: 0; color: #ebdbb2; font: inherit; background: #35312f; border: 1px solid #928374; cursor: pointer; }
    .titlebar button:hover { background: #45413f; border-color: #c6b58f; }
  ...
  private defaultShortcut(): Shortcut { return defaultShortcut(); }
  private newShellShortcut() { return this.shortcuts.find(shortcut => shortcut.action === "new-shell") || this.defaultShortcut(); }
  private matchesShortcut(event: KeyboardEvent, shortcut: Shortcut) { return matchesShortcut(event, shortcut); }
  ...
  private close(id: string) { const item = this.windows.find(window => window.id === id); if (!item) return; for (const tab of item.tabs) this.destroySession(tab.id); this.windows = this.windows.filter(window => window.id !== id); this.persistence = this.persistence.then(() => this.client.deleteWindow(id)).catch(() => undefined); }
  ...
  private moveTab(sourceWindowId: string, targetWindowId: string | undefined, tabId: string,
    ...
    if (!sourceCopy.tabs.length) this.persistence = this.persistence.then(() => this.client.deleteWindow(sourceWindowId)).catch(() => undefined);
  ...
  private moveWindow(event: PointerEvent, id: string) {
    ...
    this.editingWindow = true; this.focusWindow(id); const startX = event.clientX; const startY = event.clientY; const originX = item.x; const originY = item.y;
  ...
  private resizeWindow(event: PointerEvent, id: string) {
    ...
    const frame = this.renderRoot.querySelector<HTMLElement>(`[data-window="${id}"]`);
    const bounds = frame?.getBoundingClientRect();
    this.editingWindow = true; this.focusWindow(id); const startX = event.clientX; const startY = event.clientY; const width = bounds?.width ?? item.width; const height = bounds?.height ?? item.height;
