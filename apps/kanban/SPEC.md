...
- Card details show available and assigned ordinary tags as drag-and-drop groups; dropping into the
  item group assigns a tag and dropping back into the available group removes it.
...
- Each card displays its latest timestamped log entry and exposes its complete item log in its details.
  Descriptions from installations predating the log
