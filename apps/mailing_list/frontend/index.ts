...
export async function mount(root: HTMLElement): Promise<void> {
  ...
  const price = new Intl.NumberFormat(undefined, { style: "currency", currency: offering.currency }).format(
    offering.amount_minor / 100,
  );
  render(
    html`<style>
        * {
      ...
      ><x-console-shell class="frame"
        ><x-utility-rail slot="header"
          ><strong>DISPATCH LEDGER</strong><span>PAID SUBSCRIBER INTAKE</span></x-utility-rail
        >
        <section class="body">
          <x-console-pane title="MEMBERSHIP" tone="green"
            ><div class="card">
              <strong>Independent dispatch</strong>
              <div class="price">${price} / ${offering.interval}</div>
              <form id="enroll">
                <label
                  >Email address<input id="email" type="email" required autocomplete="email" maxlength="320" /></label
                ><x-command-button id="submit">CONTINUE TO PAYMENT</x-command-button>
              </form>
              <div id="message" class="message" role="status" aria-live="polite"></div></div
          ></x-console-pane>
        </section>
        <x-status-rail slot="footer"
          ><x-status-indicator label="PAYMENT GATEWAY READY"></x-status-indicator></x-status-rail
      ></x-console-shell>`,
    root,
  );
  ...
  form.addEventListener("submit", async (event) => {
    ...
    if (!response.ok) {
      message.textContent = result.error ?? "CHECKOUT FAILED";
      return;
    }
